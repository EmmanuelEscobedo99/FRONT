    ...
    <div className="modal modal-open px-4">
      <section className="modal-box w-full max-w-2xl">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={closeModal} className="btn btn-sm btn-circle btn-ghost">X</button>
        </div>
          ...
          loading ? (
            ...
            <>
              <div className="space-y-4 max-h-96 flex flex-col gap-4 overflow-y-auto rounded">
                {cart.map(item => (
                  <div key={item._id} className="flex items-center flex-wrap md:flex-row md:items-center gap-4 rounded-lg">
                    <img className="w-20 h-20 object-cover aspect-square" src={item.imageUrl} alt={item.name}></img>
                    <div className="flex-1 flex flex-col md:flex-row md:justify-between md:items-center gap-2">
                      ...
                      <div className="flex items-center gap-3 mt-2 md:mt-0">
                        <div className="flex items-center rounded-lg">
                          ...
                          }} className="p-2 border rounded" disabled={loading || item.quantity <= 1}>
                            <FaMinus size={12} />
                          </button>
